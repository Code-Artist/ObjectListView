...
namespace BrightIdeasSoftware
{
	...
	/// <summary>Class used to capture window messages for the header of the list view control.</summary>
	public class HeaderControl : NativeWindow
	{
		/// <summary>Create a header control for the given ObjectListView.</summary>
		...
		/// <summary>Return the index of the column under the current cursor position, or -1 if the cursor is not over a column</summary>
		/// <returns>Index of the column under the cursor, or -1</returns>
		public Int32 ColumnIndexUnderCursor
		{
			get
			{
		...
		/// <summary>Return the Windows handle behind this control</summary>
		/// <remarks>
		...
		/// <summary>Gets the index of the column under the cursor if the cursor is over it's checkbox</summary>
		protected Int32 GetColumnCheckBoxUnderCursor()
		{
			Point pt = this.ScrolledCursorPosition;

			Int32 columnIndex = NativeMethods.GetColumnUnderPoint(this.Handle, pt);
			return this.IsPointOverHeaderCheckBox(columnIndex, pt) ? columnIndex : -1;
		}

		/// <summary>Gets the client rectangle for the header</summary>
		...
		/// <summary>Return true if the given point is over the checkbox for the given column.</summary>
		/// <param name="columnIndex"></param>
		/// <param name="pt"></param>
		/// <returns></returns>
		protected Boolean IsPointOverHeaderCheckBox(Int32 columnIndex, Point pt)
		{
		...
		/// <summary>Gets whether the cursor is over a "locked" divider, i.e. one that cannot be dragged by the user.</summary>
		protected Boolean IsCursorOverLockedDivider
		{
			get
			{
				Point pt = this.ScrolledCursorPosition;
				Int32 dividerIndex = NativeMethods.GetDividerUnderPoint(this.Handle, pt);
		...
		/// <summary>Gets or sets the listview that this header belongs to</summary>
		protected ObjectListView ListView { get; set; }

		/// <summary>Gets the maximum height of the header. -1 means no maximum.</summary>
		public Int32 MaximumHeight => this.ListView.HeaderMaximumHeight;

		/// <summary>Gets the minimum height of the header. -1 means no minimum.</summary>
		public Int32 MinimumHeight => this.ListView.HeaderMinimumHeight;

		/// <summary>Get or set the ToolTip that shows tips for the header</summary>
		...
		/// <summary>Gets or sets whether the text in column headers should be word wrapped when it is too long to fit within the column</summary>
		...
		/// <summary>Calculate how height the header needs to be</summary>
		/// <returns>Height in pixels</returns>
		protected Int32 CalculateHeight(Graphics g)
		{
			TextFormatFlags flags = this.TextFormatFlags;
			Int32 columnUnderCursor = this.ColumnIndexUnderCursor;
			Single height = this.MinimumHeight;
			...
			return this.MaximumHeight == -1 ? (Int32)height : Math.Min(this.MaximumHeight, (Int32)height);
		}

		private Single CalculateColumnHeight(Graphics g, OLVColumn column, TextFormatFlags flags, Boolean isHot, Int32 i)
		{
			...
			const Int32 fudge = 9; // 9 is a magic constant that makes it perfectly match XP behavior
			if(!this.WordWrap)
		...
		/// <summary>Get the bounds of the checkbox against the given column</summary>
		...
		/// <summary>Should the given column be drawn with a checkbox against it?</summary>
		/// <param name="column"></param>
		/// <returns></returns>
		public Boolean HasCheckBox(OLVColumn column)
			=> column.HeaderCheckBox || column.HeaderTriStateCheckBox;

		/// <summary>Should the given column show a sort indicator?</summary>
		/// <param name="column"></param>
		/// <returns></returns>
		protected Boolean HasSortIndicator(OLVColumn column)
		{
		...
		/// <summary>Should the given column be drawn with a filter indicator against it?</summary>
		/// <param name="column"></param>
		/// <returns></returns>
		protected Boolean HasFilterIndicator(OLVColumn column)
			=> (this.ListView.UseFiltering && this.ListView.UseFilterIndicator && column.HasFilterIndicator);

		/// <summary>Should the given column show a non-themed sort indicator?</summary>
		/// <param name="column"></param>
		/// <returns></returns>
		protected Boolean HasNonThemedSortIndicator(OLVColumn column)
		{
			if(!this.ListView.ShowSortIndicators)
				return false;
		...
		/// <summary>Return the bounds of the item with the given index</summary>
		/// <param name="itemIndex"></param>
		/// <returns></returns>
		public Rectangle GetItemRect(Int32 itemIndex)
		{
			const Int32 HDM_FIRST = 0x1200;
			const Int32 HDM_GETITEMRECT = HDM_FIRST + 7;
			NativeMethods.RECT r = new NativeMethods.RECT();
			NativeMethods.SendMessageRECT(this.Handle, HDM_GETITEMRECT, itemIndex, ref r);
			return Rectangle.FromLTRB(r.left, r.top, r.right, r.bottom);
		}

		/// <summary>Return the bounds within which the given column will be drawn</summary>
		/// <param name="itemIndex"></param>
		/// <returns></returns>
		public Rectangle GetHeaderDrawRect(Int32 itemIndex)
		{
		...
		/// <summary>Force the header to redraw by invalidating it</summary>
		public void Invalidate()
			=> NativeMethods.InvalidateRect(this.Handle, 0, true);

		/// <summary>Force the header to redraw a single column by invalidating it</summary>
		...
		/// <summary>Create a native tool tip control for this listview</summary>
		...
		protected override void WndProc(ref Message m)
		{
			...
			const Int32 WM_SETCURSOR = 0x20;
			const Int32 WM_NOTIFY = 0x4E;
			const Int32 WM_MOUSEMOVE = 0x200;
			const Int32 WM_LBUTTONDOWN = 0x201;
			const Int32 WM_LBUTTONUP = 0x202;
			const Int32 WM_MOUSELEAVE = 675;
			const Int32 HDM_FIRST = 0x1200;
			const Int32 HDM_LAYOUT = (HDM_FIRST + 5);
		...
		private Boolean HandleReflectNotify(ref Message m)
		{
		...
		/// <summary>Handle the LButtonDown windows message</summary>
		/// <param name="m"></param>
		/// <returns></returns>
		protected Boolean HandleLButtonDown(ref Message m)
		{
		...
		private Int32 columnIndexCheckBoxMouseDown = -1;

		/// <summary>Handle the LButtonUp windows message</summary>
		/// <param name="m"></param>
		/// <returns></returns>
		protected Boolean HandleLButtonUp(ref Message m)
		{
		...
		/// <summary>Handle the SetCursor windows message</summary>
		/// <param name="m"></param>
		/// <returns></returns>
		protected Boolean HandleSetCursor(ref Message m)
		{
		...
		/// <summary>Handle the MouseMove windows message</summary>
		/// <param name="m"></param>
		/// <returns></returns>
		protected Boolean HandleMouseMove(ref Message m)
		{
			...
			Int32 columnIndex = this.ColumnIndexUnderCursor;
			...
			Int32 checkBoxUnderCursor = this.GetColumnCheckBoxUnderCursor();
		...
		private Int32 columnShowingTip = -1;
		private Int32 lastCheckBoxUnderCursor = -1;

		/// <summary>Handle the MouseLeave windows message</summary>
		/// <param name="m"></param>
		/// <returns></returns>
		protected Boolean HandleMouseLeave(ref Message m)
		{
		...
		/// <summary>Handle the Notify windows message</summary>
		/// <param name="m"></param>
		/// <returns></returns>
		...
		/// <summary>Handle the CustomDraw windows message</summary>
		/// <param name="m"></param>
		/// <returns></returns>
		internal virtual Boolean HandleHeaderCustomDraw(ref Message m)
		{
			...
			case CDDS_ITEMPREPAINT:
				Int32 columnIndex = nmcustomdraw.dwItemSpec.ToInt32();
				OLVColumn column = this.ListView.GetColumn(columnIndex);
				...
				{
					const Int32 CDIS_SELECTED = 1;
					Boolean isPressed = ((nmcustomdraw.uItemState & CDIS_SELECTED) == CDIS_SELECTED);

					// We don't need to modify this based on checkboxes, since there can't be checkboxes if we are here
					Boolean isHot = columnIndex == this.ColumnIndexUnderCursor;
		...
		private Boolean cachedNeedsCustomDraw;
		...
		/// <param name="m"></param>
		/// <returns></returns>
		protected Boolean HandleLayout(ref Message m)
		{
			...
			{
				g.TextRenderingHint = ObjectListView.TextRenderingHint;
				Int32 height = this.CalculateHeight(g);
		...
		/// <summary>Handle when the underlying header control is destroyed</summary>
		/// <param name="m"></param>
		/// <returns></returns>
		protected Boolean HandleDestroy(ref Message m)
		{
			if(this.toolTip != null)
				this.toolTip.Showing -= new EventHandler<ToolTipShowingEventArgs>(this.ListView.HeaderToolTipShowingCallback);
		...
		/// <summary>Does this header need to be custom drawn?</summary>
		/// <remarks>Word wrapping and colored text require custom drawing. Funnily enough, we can change the font natively.</remarks>
		protected Boolean NeedsCustomDraw()
		{
			if(this.WordWrap)
				return true;
		...
		private Boolean NeedsCustomDraw(HeaderFormatStyle style)
		{
			if(style == null)
				return false;
		...
		private Boolean NeedsCustomDraw(HeaderStateStyle style)
		{
			if(style == null)
				return false;
		...
		/// <summary>Draw one cell of the header</summary>
		/// <param name="g"></param>
		/// <param name="columnIndex"></param>
		/// <param name="itemState"></param>
		protected void CustomDrawHeaderCell(Graphics g, Int32 columnIndex, Int32 itemState)
		{
			OLVColumn column = this.ListView.GetColumn(columnIndex);

			Boolean hasCheckBox = this.HasCheckBox(column);
			Boolean isMouseOverCheckBox = columnIndex == this.lastCheckBoxUnderCursor;
			Boolean isMouseDownOnCheckBox = isMouseOverCheckBox && Control.MouseButtons == MouseButtons.Left;
			Boolean isHot = (columnIndex == this.ColumnIndexUnderCursor) && (!(hasCheckBox && isMouseOverCheckBox));

			const Int32 CDIS_SELECTED = 1;
			Boolean isPressed = ((itemState & CDIS_SELECTED) == CDIS_SELECTED);
		...
		private Rectangle DrawCheckBox(Graphics g, Rectangle r, CheckState checkState, Boolean isDisabled, Boolean isHot, Boolean isPressed)
		{
			...
			Int32 newX = checkBoxBounds.Right + 3;
			r.Width -= (newX - r.X);
			r.X = newX;
		...
		private Rectangle CalculateCheckBoxBounds(Graphics g, Rectangle cellBounds)
		{
			...
			Int32 topOffset = (cellBounds.Height - checkBoxSize.Height) / 2;
			return new Rectangle(cellBounds.X + 3, cellBounds.Y + topOffset, checkBoxSize.Width, checkBoxSize.Height);
		}

		private CheckBoxState GetCheckBoxState(CheckState checkState, Boolean isDisabled, Boolean isHot, Boolean isPressed)
		{
		...
		/// <summary>Draw a background for the header, without using Themes.</summary>
		/// <param name="g"></param>
		/// <param name="r"></param>
		/// <param name="columnIndex"></param>
		...
		protected void DrawUnthemedBackground(Graphics g, Rectangle r, Int32 columnIndex, Boolean isPressed, Boolean isHot, HeaderStateStyle stateStyle)
		{
		...
		/// <summary>Draw a more-or-less pure themed header background.</summary>
		/// <param name="g"></param>
		/// <param name="r"></param>
		/// <param name="columnIndex"></param>
		/// <param name="isPressed"></param>
		/// <param name="isHot"></param>
		protected void DrawThemedBackground(Graphics g, Rectangle r, Int32 columnIndex, Boolean isPressed, Boolean isHot)
		{
			Int32 part = 1; // normal item
			if(columnIndex == 0 &&
		...
		/// <summary>Draw a sort indicator using themes</summary>
		/// <param name="g"></param>
		/// <param name="r"></param>
		...
		/// <summary>Draw a sort indicator without using themes</summary>
		/// <param name="g"></param>
		/// <param name="r"></param>
		/// <returns></returns>
		protected Rectangle DrawUnthemedSortIndicator(Graphics g, Rectangle r)
		{
			...
			const Int32 triangleWidth = 16;
			const Int32 midX = triangleWidth / 2;
			const Int32 midY = (triangleHeight / 2) - 1;
			const Int32 deltaX = midX - 2;
			const Int32 deltaY = deltaX / 2;
		...
		/// <summary>Draw an indication that this column has a filter applied to it</summary>
		/// <param name="g"></param>
		/// <param name="r"></param>
		/// <returns></returns>
		protected Rectangle DrawFilterIndicator(Graphics g, Rectangle r)
		{
			Int32 width = this.CalculateFilterIndicatorWidth(r);
			if(width <= 0)
				return r;

			Image indicator = Resources.ColumnFilterIndicator;
			Int32 x = r.Right - width;
			Int32 y = r.Top + (r.Height - indicator.Height) / 2;
		...
		private Int32 CalculateFilterIndicatorWidth(Rectangle r)
		{
		...
		/// <summary>Draw the header's image and text</summary>
		/// <param name="g"></param>
		/// <param name="r"></param>
		/// <param name="column"></param>
		/// <param name="stateStyle"></param>
		...
		private void DrawImageAndText(Graphics g, Rectangle r, OLVColumn column, TextFormatFlags flags, Font f,
			Color color, Int32 imageTextGap)
		{
			...
			Int32 imageY = r.Top + ((r.Height - column.ImageList.ImageSize.Height) / 2);
			Int32 imageX = textRect.Left;
			if(column.HeaderTextAlignOrDefault == HorizontalAlignment.Center)
		...
		/// <summary>Return the header format that should be used for the given column</summary>
		/// <param name="column"></param>
		/// <returns></returns>
		...
		/// <summary>What style should be applied to the header?</summary>
		/// <param name="column"></param>
		/// <param name="isHot"></param>
		/// <param name="isPressed"></param>
		/// <returns></returns>
		protected HeaderStateStyle CalculateStateStyle(OLVColumn column, Boolean isHot, Boolean isPressed)
		{
		...
		/// <summary>What font should be used to draw the header text?</summary>
		/// <param name="column"></param>
		/// <param name="isHot"></param>
		/// <param name="isPressed"></param>
		/// <returns></returns>
		protected Font CalculateFont(OLVColumn column, Boolean isHot, Boolean isPressed)
		{
			HeaderStateStyle stateStyle = this.CalculateStateStyle(column, isHot, isPressed);
			return stateStyle.Font ?? this.ListView.Font;
		}

		/// <summary>What flags will be used when drawing text</summary>
		...
		/// <summary>Perform a HitTest for the header control</summary>
		/// <param name="x"></param>
		/// <param name="y"></param>
		/// <returns>Null if the given point isn't over the header</returns>
		internal OlvListViewHitTestInfo.HeaderHitTestInfo HitTest(Int32 x, Int32 y)
		{
