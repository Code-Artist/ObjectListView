...
namespace BrightIdeasSoftware
{
	...
	/// <summary>A delegate that creates an editor for the given value</summary>
	...
	public class EditorRegistry
	{
		...
		/// <summary>Create an EditorRegistry</summary>
		...
		/// <summary>Register that values of 'type' should be edited by instances of 'controlType'.</summary>
		...
		public void Register(Type type, Type controlType)
		{
			...
			{
				return controlType.InvokeMember(String.Empty, BindingFlags.CreateInstance, null, null, null) as Control;
			});
		}

		/// <summary>Register the given delegate so that it is called to create editors for values of the given type</summary>
		/// <param name="type">The type of value to be edited</param>
		...
		/// <summary>Register a delegate that will be called to create an editor for values that have not been handled.</summary>
		...
		/// <summary>Register a delegate that will be given a chance to create a control before any other option is considered.</summary>
		...
		/// <summary>Remove the registered handler for the given type</summary>
		...
		public Control GetEditor(Object model, OLVColumn column, Object value)
		{
			...
			if(type != null && this._creatorMap.TryGetValue(type, out EditorCreatorDelegate d))
			{
				editor = d(model, column, value);
				if(editor != null)
					return editor;
			}
		...
		/// <summary>Create and return an editor that will edit values of the given type</summary>
		/// <param name="type">A enum type</param>
		protected Control CreateEnumEditor(Type type)
			=> new EnumCellEditor(type);
